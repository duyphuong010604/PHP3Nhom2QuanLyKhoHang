...
class OutboundShipmentCreate extends Component
{
      // newProduct
      use LivewireAlert;
  ...
  public function updatingCustomerId($value)
  {
  ...
  public function validateStock($products) {
    foreach ($products as $product) {
        $stock = Stock::where('product_id', $product['product_id'])->sum('quantity');
...
//   dd($this->totalAmount);
  foreach ($this->products as $index=> $product) {
            $stock = Stock::where('product_id', $product['product_id'])->sum('quantity');
            if ($product['quantity'] > $stock) {
                $this->addError('products.' . $index . '.quantity', 'Số lượng sản phẩm vượt quá số lượng tồn kho.');
                return;
            }
        }
          ...
          foreach ($this->products as $product) {
              OutboundShipmentDetails::create([
          ...
          $this->alert('success', 'Đợt xuất hàng đã được lưu thành công!', [
              'position' => 'top-end',
              'timer' => 3000,
              'toast' => true,
              'text' => '',
  ...
  $this->h1 = $index;
    ...
    {
        return view('livewire.outbound-shipments.outbound-shipment-create');
    }
}
